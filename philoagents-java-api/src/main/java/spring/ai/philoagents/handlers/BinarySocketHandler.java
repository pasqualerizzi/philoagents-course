...
public class BinarySocketHandler extends BinaryWebSocketHandler {
    ...
    public BinarySocketHandler(OpenAiChatModel chatModel, MemorySaver memorySaver, PhilosopherService philosopherService) {
    ...
    public void handleBinaryMessage(WebSocketSession session, BinaryMessage message)
        ...
        // Convert binary payload to string (assuming UTF-8 encoding)
        ...
        var result = agent.invoke(Map.<String, Object>of(PhilosopherState.PN_KEY, philosopher.getId(),
                PhilosopherState.PS_KEY, philosopher.getStyle(), PhilosopherState.PP_KEY, philosopher.getPerspective(),
                "messages", new UserMessage(value.get("message")))).orElseThrow();
